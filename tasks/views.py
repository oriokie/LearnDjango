...
def read (request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            sdf = pd.read_fwf(excel_fileSTAT ,header=None, widths=[13,20,15,9,32,16], index=False)
            ...
            with open("Recon.xlsx", "rb") as f:
                response = HttpResponse(f.read(), content_type="application/vnd.ms-excel",status=200)
    ...
    return render(request, 'tasks/read.html',  {'form': form})
